...
short GetOutputCode(float x, float y)
{
	...
	if (x < Viewport::Get()->GetMinX()) code |= BIT_LEFT;
	else if (x > Viewport::Get()->GetMaxX()) code |= BIT_RIGHT;
	else if (y < Viewport::Get()->GetMinY()) code |= BIT_TOP;
	else if (y > Viewport::Get()->GetMaxY()) code |= BIT_BOTTOM;
...
bool Clipper::ClipLine(Vertex& v0, Vertex& v1)
{
	...
	while (true)
	{
		...
		{
			float t = 0.0f;
			short outcodeOut = codeV1 > codeV0 ? codeV1 : codeV0; // findout which one is outside the screen
			...
			else
			{
				...
				codeV0 = GetOutputCode(v1.pos.x, v1.pos.y);
			}
		}
	}
